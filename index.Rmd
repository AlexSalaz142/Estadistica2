...
```
Las etiquetas no muestran un orden, por lo que hay que reconfigurarlo:

```{r}
original=aggregate(.~ diana, data=dataClus,mean)
original[order(original$urb_pop),]
```
```{r}
dataClus$diana=dplyr::recode(dataClus$diana, `3` = 1, `1`=2,`2`=3,`4`=4)
```
