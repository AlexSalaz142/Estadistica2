...
```
De ello se saca que el porcentaje de población urbana, de embarazo adolescente, de gasto en educación y la tasa de mortalidad se agrupan en un factor, el cual podríamos considerar como el factor social. Por el otro lado, el porcentaje de fuerza laboral y de desempleo se agrupan en un segundo factor, el cual habíamos previsto como el factor laboral. Por último, el porcentaje de exportación de metales y minerales y la tasa de matrícula escolar se quedan fuera de la agrupación, por lo que, teóricamente, podría descartarse del estudio. 
...
```{r}
library(BBmisc)
datafinal[,c(2:11)]= normalize(datafinal[,c(2:11)],method='standardize')
```

Luego evidenciamos las correlaciones:
```{r}
cor(datafinal[,c(2:11)]) 
```
